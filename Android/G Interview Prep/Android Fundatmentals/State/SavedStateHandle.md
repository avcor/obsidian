...
- Google Introduced this because 
	...
	- The use depends on where the state is held and logic that it requires
		...
		- For state that is required in UI use [[Save Instance#`onSavedInstanceState(outState Bundle)`]] or `rememberSaveable` in compose.
...
## No Large Objects in it
It is backed by bundle, which is send across binder transaction. Binder has strict size limit.
Large Object increase the risk 
- `TransactionTooLargeException`
- Partial or failed state restoration 
More importantly large objects like network responses are re-derivable, non essential UI state. Persisting them would violate the principle of storing the minimal user-critical state. 
## How data is stored 
```
SavedStateHandle
|
